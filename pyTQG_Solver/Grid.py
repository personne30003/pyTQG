...
class Grid:
    def __init__(self, params):
        ...
        ##########Eventuelles matrices de différenciation (seulement pour configuration "canal"###########################################
        self.__Dy = None
        self.__Dy_BC = None
        self.__D2y = None
        self.__D2y_BC = None
        ...
        if self.geometry == 'biperiodic':
            ...
            self.__d2y_1D = lambda yy : Fourier.Fourier_deriv(yy, order = 2, a = self.__x_bounds[0], b = self.__x_bounds[1])
            ...
            self.__d2x_BC = self.__d2x_1D
            self.__d2y_BC = lambda yy : self.__D2y_BC@yy
            ...
            self.__int_x_1D = lambda xx : Fourier.Fourier_quad(xx, self.__dx)
            self.__int_y_1D = lambda yy : Chebyshev.Cheb_quad(yy, a = self.__x_bounds[0], b = self.__x_bounds[1])
        ...
        self.integrate_all_domain = self.__integrate_func()
        self.int_cum_x = self.__apply_along_axis(self.__int_cum_x_1D, "x")
        self.int_cum_y = self.__apply_along_axis(self.__int_cum_y_1D, "y")
    ...
    def integrate(self, val, axis):
        "Intègre un array de taille (Nx, Ny), sur les axes: 'x', 'y' (intégrale cumulée) ou 'all' (intégrale sur tout le domaine)"
        pass
    ...
    def jacobien(self,val_A, val_B, BC_A = False, BC_B = True):
        ...
        dx_B = getattr(self.__liste_deriv_y, key_BC_B)(val_B)
    ...
    def psi_from_U(self, U):
        "Calcule la fonction de courant à partir de la vitesse zonale (provisoire, à voir si je mets ça dans les modèles)"
        pass
###########Fonctions privées########################################################
    def __apply_along_axis(self, func, axis):
        ...
        def func_along_axis(x):
            self.__check_shape(self, x)
            x_res = np.zeros_like(x)
            
            for i in range(0, N_ax):
                x_i = np.take_along_axis(x, np.array([i]), axis = axis_sel).copy()
                np.put_along_axis(x_res, i, func(x_i) ,axis_sel)
            return x_res
    ...
    def __integrate_func(self):
        "Pour intégrer sur tout un intervalle"
